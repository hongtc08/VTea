...
public class POSController implements Initializable {
    ...
    private void loadProductsFromDB(Connection conn) throws SQLException {
        String sql = "SELECT p.*, c.name as category_name FROM fnb_management.product p " +
                "LEFT JOIN fnb_management.category c ON p.category_id = c.category_id " +
    ...
    private void loadCategoriesFromDB(Connection conn) throws SQLException {
        ...
        String sql = "SELECT DISTINCT name FROM fnb_management.category ORDER BY name";
        try (Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery(sql)) {
    ...
    private VBox buildProductCard(Product product) {
        ...
        Label cat = new Label(product.getImageUrl() != null ? product.getImageUrl() : "");
    ...
    private void handleCheckout() {
        if (cartMap.isEmpty()) {
            showAlert(Alert.AlertType.WARNING, "Lỗi", "Giỏ hàng đang trống. Vui lòng chọn món trước!");
            return;
        }
        ...
        confirm.setContentText(String.format("Tạm tính: %s\nVAT (10%%): %s\n\nTổng cộng: %s",
        ...
        confirm.showAndWait().ifPresent(result -> {
            if (result == ButtonType.OK) {
                ...
                cartMap.clear();
                updateCartDisplay();
                showAlert(Alert.AlertType.INFORMATION, "Thành công", "Đơn hàng đã được thanh toán!\nTổng: " + formatCurrency(total));
            }
        });
    }

    private void saveOrderToDB(BigDecimal total) {
        String sqlOrder = "INSERT INTO fnb_management.`order` (user_id, total_amount, status, payment_method, created_at) VALUES (?, ?, 'COMPLETED', 'CASH', NOW())";
        try (Connection conn = DBConnection.getConnection()) {
            if (conn == null) return;
            ...
            try (PreparedStatement ps = conn.prepareStatement(sqlOrder, Statement.RETURN_GENERATED_KEYS)) {
                ...
                conn.rollback();
                showAlert(Alert.AlertType.ERROR, "Lỗi Database", "Không thể lưu hóa đơn: " + e.getMessage());
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    private void saveOrderDetails(Connection conn, int orderId) throws SQLException {
        String sql = "INSERT INTO fnb_management.order_detail (order_id, product_id, quantity, unit_price) VALUES (?, ?, ?, ?)";
